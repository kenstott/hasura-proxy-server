...
export const generateMessageFromObjectType = (type?: GraphQLObjectType | GraphQLInputObjectType | null, messages?: Record<string, Partial<Message | Enum>>): { message?: Message | Enum, supportingMessages?: Record<string, Partial<Message | Enum>> } => {
  if (type) {
    messages = messages ?? {}
    ...
    for (const [name, queryField] of queryFields) {
      ...
      for (const arg of queryField.args ?? [] as GraphQLArgument[]) {
        const argResponse = generateFieldFromType({ name: (arg.name as string), type: arg.type as GraphQLType, field: {}, messages })
